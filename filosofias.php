...
<nav class="navbar">
    ...
    <div class="titulo">
        ...
        <label>Escreva seus pensamentos abaixo</label>
    </div>
    <form action="filosofias.php" method="post">
        ...
        <div class="form-group">
            <textarea name="text_filo" placeholder="Texto Filosófico" rows="6" required></textarea>
        </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $titulo = $_POST['titulo'] ?? '';
        $autor = $_POST['autor'] ?? '';
        $text_filo = $_POST['text_filo'] ?? '';

        if ($titulo && $autor && $text_filo) {
            // Prepara o SQL com placeholders
            $stmt = $conn->prepare("INSERT INTO filosofia (titulo, autor, text_filo) VALUES (?, ?, ?)");

            if ($stmt) {
                // Vincula os parâmetros (s = string, s = string, s = string)
                $stmt->bind_param("sss", $titulo, $autor, $text_filo);

                if ($stmt->execute()) {
                    echo "<p class='success'>✅ Filosofia inserida com sucesso!</p>";
                } else {
